...
def run_nmf(
    ...
    NMF uses various solvers (specified in ``algo`` parameter), in either batch or minibatch mode (specified in ``mode`` parameter), to minimize this objective function.
    ...
    mode: ``str``, optional, default: ``batch``
        Learning mode. Choose from ``batch``, ``minibatch``, and ``dataloader``.
        Notice that ``minibatch`` and ``dataloader`` only work when ``beta=2.0``. For other beta loss, they switch back to ``batch`` method.
            - ``batch``: Standard batch learning, loading the entire matrix into memory.
            - ``minibatch``: Online learning using mini-batches of samples.
            - ``dataloader``: Online learning using a PyTorch DataLoader for mini-batch iteration, which can be more memory-efficient for large datasets. Only supported for the HALS solver (``algo='hals'`` or ``algo='halsvar'``); for other solvers, falls back to ``minibatch``.
    ...
    minibatch_max_epoch: ``int``, optional, default: ``20``
        The maximum number of epochs (passes over all data) for minibatch learning.
    ...
    minibatch_max_iter: ``int``, optional, default: ``200``
        The maximum number of iterations for updating H or W per minibatch.
    minibatch_h_tol: ``float``, optional, default: 0.05
    ...
    >>> H, W, err = run_nmf(X, n_components=20, init='random', algo='mu', mode='minibatch')
    """
    ...
    if mode not in {'batch', 'minibatch', 'dataloader'}:
        raise ValueError("Parameter mode must be a valid value from ['batch', 'minibatch','dataloader']!")
    if beta_loss != 2 and (mode == 'minibatch' or mode == 'dataloader'):
        print("Cannot perform minibatch update when beta not equal to 2. Switch to batch update method.")
        mode = 'batch'

    if mode == 'dataloader' and algo not in {'hals', 'halsvar'}:
        print(f"Dataloader mode is only supported for HALS solver (algo='hals' or 'halsvar'), but got algo='{algo}'. Falling back to minibatch mode.")
        mode = 'minibatch'
    ...
    if mode == 'batch':
        kwargs['max_epoch'] = batch_max_epoch
        if algo == 'mu':
        ...
        kwargs['max_epoch'] = minibatch_max_epoch
        ...
        raise ValueError("mode must be batch, minibatch, or dataloader")
...
def integrative_nmf(
    ...
    lam: float = 5.,
    fp_precision: Union[str, torch.dtype] = "float",
    batch_max_epoch: int = 200,
    batch_hals_tol: float = 0.0008,
    batch_hals_max_iter: int = 200,
    minibatch_max_epoch: int = 20,
    minibatch_size: int = 5000,
    minibatch_max_iter: int = 200,
    minibatch_h_tol: float = 0.01,
    minibatch_v_tol: float = 0.1,
    minibatch_w_tol: float = 0.01,
    ...
    iNMF uses various solvers (specified in ``algo`` parameter), either in batch or minibatch mode (specified in ``mode`` parameter), to minimize this objective function.
    ...
    init: ``str``, optional, default: ``normal``
        ...
        If ``None``, use ``norm`` for minibatch mode, while ``uniform`` for batch mode, in order to achieve best performance.
    algo: ``str``, optional, default: ``halsvar``
    ...
    mode: ``str``, optional, default: ``batch``
        Learning mode. Choose from ``batch``, ``minibatch``, and ``dataloader``.
        Note: ``dataloader`` mode is not yet supported for integrative NMF and will fall back to ``minibatch``.
    tol: ``float``, optional, default: ``1e-4``
        The toleration used for convergence check.
    n_jobs: ``int``, optional, default: ``-1``
        Number of cpu threads to use. If -1, use PyTorch's default setting.
    ...
    fp_precision: ``str``, optional, default: ``float``
        ...
        If ``float``, set precision to ``torch.float``; if ``double``, set precision to ``torch.double``.
        Alternatively, choose Pytorch's `torch dtype <https://pytorch.org/docs/stable/tensor_attributes.html>`_ of your own.
    batch_max_epoch: ``int``, optional, default: ``200``
        The maximum number of iterations to perform for batch learning.
    ...
    minibatch_max_iter: ``int``, optional, default: ``200``
        The maximum number of iterations for updating H or W per minibatch.
    minibatch_h_tol: ``float``, optional, default: ``0.01``
        The tolerance for updating H per minibatch.
    minibatch_v_tol: ``float``, optional, default: ``0.1``
        The tolerance for updating V per minibatch.
    minibatch_w_tol: ``float``, optional, default: ``0.01``
        The tolerance for updating W per minibatch.
    ...
    >>> H, W, V, err = integrative_nmf(X, n_components=20, algo='bpp', mode='minibatch')
    """
    ...
    if algo not in {'halsvar', 'mu', 'bpp'}:
        raise ValueError("Parameter algo must be a valid value from ['halsvar', 'mu', 'bpp']!")
    if mode not in {'batch', 'minibatch', 'dataloader'}:
        raise ValueError("Parameter mode must be a valid value from ['batch', 'minibatch', 'dataloader']!")

    if mode == 'dataloader':
        print("Dataloader mode not yet supported for integrative NMF. Falling back to minibatch.")
        mode = 'minibatch'
